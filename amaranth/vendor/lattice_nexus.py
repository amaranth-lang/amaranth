...
class LatticeNexusPlatform(TemplatedPlatform):
    ...
    Build products:
        ...
        * ``{{name}}.bit``: binary bitstream.
        * ``{{name}}.svf``: JTAG programming vector. FIXME
    """
        ...
        **TemplatedPlatform.build_script_templates,
        ...
        "{{name}}.pdc": r"""
            ...
            {% for port_name, pin_name, attrs in platform.iter_port_constraints_bits() -%}
                ldc_set_location -site {{ '{' }}{{port_name}}{{ '}' }} {{'['}}get_ports {{pin_name}}{{']'}} 
                {% if attrs -%}
    ...
    def default_clk_constraint(self):
        if self.default_clk == "OSC":
            return Clock(150e6 / self.osc_div) # lscc/radiant/2022.1/docs/webhelp/eng/index.htm#page/Reference%20Guides/FPGA%20Libraries%20Reference%20Guide/Alphanumeric_Primitives_List.11.155.htm        
        # For HFCLKOUT (created clock): ADJUSTED_PERIOD=6666.66667ps * HF_CLK_DIV
    ...
    def create_missing_domain(self, name):
        ...
        if name == "sync" and self.default_clk is not None:
            ...
            if self.default_clk == "OSC":
                ...
                if not isinstance(self.osc_div, int) or self.osc_div < 2 or self.osc_div > 128:
                    raise ValueError("OSC divider (osc_div) must be an integer between 2 "
                                     ...
                                     .format(self.osc_div))
                clk_i = Signal()
                m.submodules += Instance("OSC", p_HF_CLK_DIV=self.osc_div, o_HFCLKOUT=clk_i)
            else:
            ...
            # LIFCL UPDATE: GSR() has a parameter (SYNCMODE) to perform synchronization of the reset to the user clock
            m.submodules += [
                ...
                Instance("GSR", p_SYNCMODE="SYNC", i_CLK=clk_i, i_GSR_N=~rst_i),
            ]
    ...
    _single_ended_io_types = [
        ...
        "HSUL12", "MIPI D-PHY LP Input (LVCMOS12)"
    ]
        ...
        def get_iddr(sclk, d, q0, q1):
            for bit in range(len(d)):
                m.submodules += Instance("IDDRX1",
                    ...
                    i_RST=Const(0),
                    i_D=d[bit],
                    o_Q0=q0[bit],  # data at positive edge
                    o_Q1=q1[bit],
                )
        ...
        def get_iddrx2(sclk, eclk, d, q0, q1, q2, q3):
            for bit in range(len(d)):
                m.submodules += Instance("IDDRX2",
                    ...
                    i_ALIGNWD=Const(0),
                    i_D=d[bit],
                    o_Q0=q0[bit], o_Q2=q2[bit], # data at positive edge
                    o_Q1=q1[bit], o_Q3=q3[bit]
                )
        ...
        def get_oddrx2(sclk, eclk, d0, d1, d2, d3, q):
            for bit in range(len(q)):
                m.submodules += Instance("ODDRX2",
                    ...
                    i_ECLK=eclk,
                    i_RST=Const(0),
                    i_D0=d0[bit], i_D1=d1[bit], i_D2=d2[bit], i_D3=d3[bit], # D0 sent first
                    o_Q=q[bit]
                )
    ...
    # FIXME While Diamond supports false path constraints; nextpnr-ecp5 does not.
