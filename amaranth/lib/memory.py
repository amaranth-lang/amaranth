...
class Memory(wiring.Component):
    ...
    This class is used to construct a memory array by first specifying its dimensions and initial
    contents using the :py:`shape`, :py:`depth`, and :py:`init` parameters, and then adding memory
    ports using the :meth:`read_port` and :meth:`write_port` methods. Because it is mutable, it
    should be created and used locally within the :ref:`elaborate <lang-elaboration>` method.
    ...
    ..
        :attr:`attrs` is intentionally left undocumented
...
class ReadPort:
    ...
    Memory read ports can be constructed by calling :meth:`Memory.read_port` or via
    :meth:`ReadPort.Signature.create() <.Signature.create>`.
...
class WritePort:
    ...
    Memory write ports can be constructed by calling :meth:`Memory.write_port` or via
    :meth:`WritePort.Signature.create() <.Signature.create>`.
